...
```{r}
library(readxl)
tab14_eng <- read_excel("~/Documents/assignments/keira/cleaned/tab14-eng.xls", skip = 4, n_max = 19)
Sys.setlocale(category = "LC_CTYPE", locale = "en_US.UTF-8")
head(tab14_eng)
print(dim(tab14_eng))
tab14_eng <- clean_names(tab14_eng)
print(colnames(tab14_eng))
selected_colnames <- c("region_of_work", "all_employees", "women", "persons_in_a_visible_minority_group","persons_with_disabilities" , "aboriginal_peoples")
head(tab14_eng)
subset_data <- tab14_eng[, selected_colnames]
subset_data <- subset_data[complete.cases(tab14_eng$region_of_work), ]
head(subset_data)
```
...
1. Drawing a bar graph showing the different  distributions of employees categories across regions of work
